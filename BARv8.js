...
if (classes.length != 10) {
  alert(
    "(BARv8) There are " +
      classes.length +
      " CRNs in your list instead of 10! BAR will not work unless this is fixed. This error message will disappear when the issue is resolved."
  );
}
if (hour < 0 || hour > 11 || minute < 0 || minute > 59) {
  alert(
    "(BARv8) The time information you entered is incorrect. This error message wil disappear when the issue is resolved."
  );
}
var url = window.location.pathname;
if (
  url.includes("P_AltPin") ||
  url.includes("P_CheckAltPin") ||
  url.includes("P_Regs")
) {
  if (document.querySelector("#apin_id") !== null) {
    ...
    document
      .querySelector("body > div.pagebodydiv > form > input[type=submit]")
      .click();
  } else {
    if (!document.querySelector(".dataentrytable")) {
      ...
      if (!cookieCheck("completedRegistration")) {
        console.log(
          "BAR > Found registration page, trying to autofill courses..."
        );
        for (var i = 0; i < 10; i++) {
          document.querySelectorAll('.dedefault > input[name="CRN_IN"]')[
            i
          ].value = classes[i];
        }
        ...
        console.log(
          "BAR > Thanks for using BARv8! Learn more at https://link.canon.click/bar-tool"
        );
      }
    }
  }
